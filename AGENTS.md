...
- **Fase actual**: Fase 5 — Frontend **en curso**. Bloques `5.0` (cableado HTTP del backend), `5.1` (base Next.js), `5.2` (cliente HTTP + TanStack Query + Zustand + Zod), `5.3` (vista diff de 3 columnas + formulario) y `5.4` (dashboard de analíticas) **completados** y verificados en navegador. El backend sirve prácticas CRUD + analyze + `analytics/error-patterns`. Fase 4 (motor de IA) completada con Gate en verde.
- **Siguiente item**: Fase 5, bloque `5.5` — tests frontend (Vitest unit, React Testing Library + axe-core y Playwright e2e), empezando por la lógica pura (`diffWords`, `mostFrequentPattern`, schemas, `ApiClient`). Diferido a Fase 6: `/analytics/progress` y `/me/*` (A9), que hoy responden `501 not_implemented`.
- **Estado**: monorepo base, contrato OpenAPI (`apps/contracts/openapi/api.yaml`), pipeline `pnpm generate`, módulo Go (`apps/backend/go.mod`, module path `github.com/c0mp1lerworld/langlint/backend`), Fases 3–4 (puertos/adaptadores Postgres, UoW, outbox + event bus; motor de IA con Structured Outputs, `AnalysisService`, `anonymizer` y `PIIHandler`) y Fase 5.0–5.4. Añadidos en la verificación manual de 5.4: CORS configurable en el backend (`APP_CORS_ALLOWED_ORIGINS`, default `http://localhost:3000`) y fix del `fetchImpl` del cliente (`Illegal invocation`). El frontend requiere `apps/frontend/.env.local` con `NEXT_PUBLIC_API_URL` (gitignored). `gen_*.go`/`gen.ts` commiteados. Dominio puro stdlib-only; `go build/vet/test` en verde; `pnpm build`/`pnpm lint` (3 apps) y `pnpm typecheck --filter=frontend` en verde.
