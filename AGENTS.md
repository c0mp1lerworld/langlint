...
- **Siguiente item**: opcional — listar el historial de sesiones (`GET /v1/study-sessions`). **Gaps pre-post-MVP cerrados** (2026-09-21): `GAP-1`/`BUG-003` (`/analytics/progress` ya responde `200`), `BUG-002` (`finish_reason=length` → `llm_output_truncated`) y `BUG-001` (run-ons subdivididos). **`9.7` cerrado** (2026-09-22): analytics del quiz con métrica separada (ledger append-only `quiz_attempts` + `analytics.QuizAttempt`/`QuizStats` + `GET /analytics/quiz`, contrato `3.4.0`). **Gate de Fase 7 en verde**: el primer run real de CI en GitHub no obtuvo errores (confirmado por el humano). Queda abierta solo la deuda de seguridad `SD-3`.
