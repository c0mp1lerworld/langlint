...
- **Fase actual**: Fase 8 — Tutor Adaptativo (post-MVP); bloques **`8.1`** (bounded context `tutor/`) y **`8.2`** (detección de debilidades) **completados**. `8.1`: `internal/domain/tutor/` con `StudySession`, `WeaknessProfile`, `Trap`, `Exercise`, `StudySessionStatus` (dominio puro 100 %, aislamiento A3). `8.2`: `domain.Window` (raíz, alias en `analytics`), evento `domain.WeaknessDetected` emitido desde `AnalysisCompletedHandler` vía outbox (umbral `analytics.WeaknessThreshold=5` + `DetectWeakness`) y `WeaknessDetectedHandler` (tutor) que construye `tutor.WeaknessProfile` consultando agregados. Quedan `8.3`–`8.4`. Historial previo: Fase 7 — Hardening; bloques **CI/CD `7.1`**, **Seguridad `7.2`** y **Documentación `7.3`** (`7.3.1`–`7.3.4`) **completados**. `7.1`: `ci.yml` (detect-changes + jobs contracts/backend/frontend, Tier 3 en nightly/push/label), `contracts.yml` (regenera Go+TS y falla por drift; valida `info.version == package.json`), `deploy-staging.yml` + Dockerfiles (backend distroless `nonroot`, frontend `standalone`) y remote cache self-hosted (`ops/docker/`). `7.2`: `security.yml` (`gosec` v2.29.0 + `govulncheck` v1.8.0, SARIF), `ops/scripts/security_audit.sh` (secretos + guard de docs/x-internal), `/docs` no expuesto verificado (test + guard) y `x-internal` N/A. Se fijó `toolchain go1.26.8` (govulncheck: 0 vulns) y se enmascaró `& 0xff` en `identifiers.go` (falso positivo gosec G115). `7.3`: `docs/ARCHITECTURE.md`, `RUNBOOK.md`, `PRODUCTION_ENV.md`, `SECRET_ROTATION.md`, `SECURITY_DEBT.md`, `API_CONTRACT.md` (gaps, AP5), `docs/bugs/` (3 bugs) y README de portafolio (es/en). El *Gate de salida* de Fase 7 sigue abierto: solo falta el primer run real de los workflows (no corren en local). Historial: Fase 6 — Provisioner y privacidad **completada; Gate de salida en verde**. Bloques `6.1` (jobs batch `refresh-aggregates`, `purge-raw-data`, `execute-deletions`), `6.2` (endpoints A9 `GET /me/data/export`, `DELETE /me/data`, `GET /me/access-log`) y `6.3` (pseudonimización HMAC-SHA256 de analytics con `APP_PSEUDONYM_SECRET`, separación de datos crudos y auditoría `ops/scripts/pii_audit.sh`) completados. Mejora post-MVP **Fase 9 — feedback profundo y práctica activa** completada en `9.1`–`9.6` (explicaciones estructuradas + quiz IA); `9.7` (analytics del quiz) queda pendiente por diseño (ver checklist 09).
- **Siguiente item**: Fase 8.3 — generación de la sesión con IA (Structured Outputs, reusando el patrón del `LLMExtractor`): teoría resumida + 3 trampas comunes + 5 ejercicios interactivos, anonimización previa al LLM (A8) y validación contra schema. Sigue pendiente cerrar el *Gate de salida* de Fase 7 con el **primer run real de CI en GitHub** (los workflows no se ejecutan en local) y el `9.7` diferido. Acción humana pendiente: crear los secrets `TURBO_API`/`TURBO_TEAM`/`TURBO_TOKEN` (y la variable `STAGING_API_URL`) para activar la caché remota y el inlining del API en staging. **Gaps pre-post-MVP cerrados** (2026-09-21): `GAP-1`/`BUG-003` (`/analytics/progress` ya responde `200`), `BUG-002` (`finish_reason=length` → `llm_output_truncated`) y `BUG-001` (run-ons subdivididos). Queda abierto solo el `9.7` (analytics del quiz, diferido por diseño) y la deuda de seguridad `SD-3`.
