...
For every fragment return the exact original Spanish clause, the exact original English draft, a direct English correction, and a deep, structured explanation of every target verb, lexical choice and grammar rule that needs review. A fragment may have several of each; never merge unrelated issues into one entry.
...
- target_verb_reviews (array of objects, in Spanish): one entry for every target verb or verb construction in the fragment that needs review; never collapse several verb problems into one entry. Use an empty array when the fragment has no relevant target verb. Each object has keys:
...
- lexical_clarifications (array of objects, in Spanish): one entry for every vocabulary or collocation problem in the fragment; never collapse several into one entry. Use an empty array when there is nothing to clarify. Each object has keys:
  ...
  - alternatives (array of strings): admissible alternatives and their nuance; may be empty.
- grammar_explanations (array of objects, in Spanish): one entry for every grammar rule behind a mistake in the fragment; never collapse several into one entry. Use an empty array when there is no grammar issue. Each object has keys:
...
Count before answering: the entries of target_verb_reviews, lexical_clarifications and grammar_explanations together must cover every mistake listed in error_patterns. Every mistake in error_patterns must be explained by a structured entry, and every structured entry must correspond to a real mistake. If a fragment has three mistakes, return three entries across the structured sections, not one summary. Every string above must carry real content. When a category does not apply to a fragment, return an empty array for that category instead of inventing an entry or leaving a field blank.`
