...
Analyze the learner's English draft against the Spanish source clause by clause. Cover the entire text: produce one fragment per meaningful clause, in the original order. Split long sentences at conjunctions (and, but, because, that, where...), relative pronouns and punctuation; never pack multiple independent clauses into a single fragment.
For every fragment return the exact original Spanish clause, the exact original English draft, a direct English correction, and structured explanations of the mistakes that need review. A fragment may have several entries of each kind; never merge unrelated issues into one entry. Keep every field to one short sentence (max 20 words): this is a report, not an essay. Do not create a separate fragment for a clause that has no mistake.
...
- target_verb_reviews (array of objects, in Spanish): one entry per target verb or verb construction in the fragment that needs review; never collapse several verb problems into one entry. Return at most three entries; if there are more, keep the three most important. Use an empty array when the fragment has no relevant target verb. Each object has keys:
...
- lexical_clarifications (array of objects, in Spanish): one entry per vocabulary or collocation problem in the fragment; never collapse several into one entry. Return at most three entries; if there are more, keep the three most important. Use an empty array when there is nothing to clarify. Each object has keys:
  ...
  - alternatives (array of strings): admissible alternatives and their nuance; may be empty.
- grammar_explanations (array of objects, in Spanish): one entry per grammar rule behind a mistake in the fragment; never collapse several into one entry. Return at most three entries; if there are more, keep the three most important. Use an empty array when there is no grammar issue. Each object has keys:
...
Prioritize: list every mistake in error_patterns, but explain in the structured sections only the most important ones (at most three entries per category and per fragment). Never repeat the same explanation across entries, and never pad a field with filler. Every structured entry must correspond to a real mistake. Every string above must carry real content. When a category does not apply to a fragment, return an empty array for that category instead of inventing an entry or leaving a field blank.`
