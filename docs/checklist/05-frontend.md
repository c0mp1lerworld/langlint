...
- [x] `5.3.5` Formulario de creación de práctica (React Hook Form + Zod) + mutación `POST /practices`. _(`features/practice/components/practice-form.tsx` con `useFieldArray` para `target_rules` y validación F8; resolver propio `lib/utils/zod-rhf.ts` (sin `@hookform/resolvers`); `useCreatePractice` invalida la lista y navega al detalle; ruta `/practices/new` y enlace "Nueva práctica" en la home. Cierra el gap de creación necesario para el e2e de `5.5.3`.)_
